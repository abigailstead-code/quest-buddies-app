/**
 * Generated by orval v8.23.0 🍺
 * Do not edit manually.
 * Api
 * Quest Road multiplayer API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * Returns server health status
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary Create a multiplayer room
 */
export const createRoomBodyNameMax = 80;

export const createRoomBodyPlayerNameMax = 40;



export const CreateRoomBody = zod.object({
  "name": zod.string().min(1).max(createRoomBodyNameMax),
  "playerName": zod.string().min(1).max(createRoomBodyPlayerNameMax)
})

export const CreateRoomResponse = zod.object({
  "id": zod.string(),
  "code": zod.string(),
  "name": zod.string(),
  "status": zod.enum(['waiting', 'playing']),
  "host": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}),
  "guest": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}).nullable(),
  "createdAt": zod.string()
})


/**
 * @summary Get a room by id
 */
export const getRoomPathRoomIdMin = 4;



export const GetRoomParams = zod.object({
  "roomId": zod.coerce.string().min(getRoomPathRoomIdMin)
})

export const GetRoomResponse = zod.object({
  "id": zod.string(),
  "code": zod.string(),
  "name": zod.string(),
  "status": zod.enum(['waiting', 'playing']),
  "host": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}),
  "guest": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}).nullable(),
  "createdAt": zod.string()
})


/**
 * @summary Join a waiting room with its invite code
 */
export const joinRoomPathRoomIdMin = 4;



export const JoinRoomParams = zod.object({
  "roomId": zod.coerce.string().min(joinRoomPathRoomIdMin)
})

export const joinRoomBodyCodeMin = 4;
export const joinRoomBodyCodeMax = 12;

export const joinRoomBodyPlayerNameMax = 40;



export const JoinRoomBody = zod.object({
  "code": zod.string().min(joinRoomBodyCodeMin).max(joinRoomBodyCodeMax),
  "playerName": zod.string().min(1).max(joinRoomBodyPlayerNameMax)
})

export const JoinRoomResponse = zod.object({
  "id": zod.string(),
  "code": zod.string(),
  "name": zod.string(),
  "status": zod.enum(['waiting', 'playing']),
  "host": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}),
  "guest": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}).nullable(),
  "createdAt": zod.string()
})


/**
 * @summary Get the shared game state
 */
export const getGameStatePathRoomIdMin = 4;



export const GetGameStateParams = zod.object({
  "roomId": zod.coerce.string().min(getGameStatePathRoomIdMin)
})




export const GetGameStateResponse = zod.object({
  "room": zod.object({
  "id": zod.string(),
  "code": zod.string(),
  "name": zod.string(),
  "status": zod.enum(['waiting', 'playing']),
  "host": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}),
  "guest": zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
}).nullable(),
  "createdAt": zod.string()
}),
  "players": zod.array(zod.object({
  "id": zod.string(),
  "name": zod.string(),
  "avatar": zod.string(),
  "color": zod.string(),
  "coins": zod.number(),
  "xp": zod.number(),
  "streak": zod.number()
})),
  "quests": zod.array(zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})),
  "rewards": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "emoji": zod.string(),
  "cost": zod.number(),
  "createdBy": zod.string(),
  "active": zod.boolean(),
  "createdAt": zod.string()
})),
  "encouragements": zod.array(zod.object({
  "id": zod.string(),
  "fromId": zod.string(),
  "toId": zod.string(),
  "message": zod.string(),
  "createdAt": zod.string()
})),
  "transactions": zod.array(zod.object({
  "id": zod.string(),
  "playerId": zod.string(),
  "amount": zod.number(),
  "kind": zod.enum(['quest', 'reward', 'bonus']),
  "label": zod.string(),
  "createdAt": zod.string()
})),
  "redemptions": zod.array(zod.object({
  "id": zod.string(),
  "rewardId": zod.string(),
  "rewardTitle": zod.string(),
  "playerId": zod.string(),
  "providerId": zod.string(),
  "cost": zod.number(),
  "redeemedAt": zod.string(),
  "sourceType": zod.enum(['standard', 'request', 'challenge']),
  "completedAt": zod.string().nullable(),
  "completedById": zod.string().nullable(),
  "sourceRequestId": zod.string().nullable(),
  "sourceChallengeId": zod.string().nullable()
  })),
  "labels": zod.array(zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "name": zod.string(),
  "color": zod.string(),
  "createdAt": zod.string()
})),
  "rewardRequests": zod.array(zod.object({
  "id": zod.string(),
  "requesterId": zod.string(),
  "responderId": zod.string(),
  "payerId": zod.string(),
  "turnPlayerId": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "offer": zod.number(),
  "status": zod.enum(['awaiting_response', 'countered', 'accepted', 'declined', 'paid']),
  "createdAt": zod.string(),
  "updatedAt": zod.string()
  })),
  "challenges": zod.array(zod.object({
  "id": zod.string(),
  "type": zod.enum(['task', 'coin']),
  "status": zod.enum(['awaiting_response', 'countered', 'accepted', 'declined', 'completed', 'succeeded', 'failed']),
  "challengerId": zod.string(),
  "recipientId": zod.string(),
  "turnPlayerId": zod.string().nullable(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "rewardTitle": zod.string().nullable(),
  "rewardCoins": zod.number().nullable(),
  "dueDate": zod.string().nullable(),
  "targetCoins": zod.number().nullable(),
  "startDate": zod.string().nullable(),
  "endDate": zod.string().nullable(),
  "acceptedAt": zod.string().nullable(),
  "completedAt": zod.string().nullable(),
  "unlockedRedemptionId": zod.string().nullable(),
  "progressCoins": zod.number(),
  "createdAt": zod.string(),
  "updatedAt": zod.string()
  })),
  "currentLevel": zod.number(),
  "weekLabel": zod.string(),
  "daysRemaining": zod.number()
})


/**
 * @summary Get a weekly Journey Bingo board
 */
export const getBingoBoardPathRoomIdMin = 4;



export const GetBingoBoardParams = zod.object({
  "roomId": zod.coerce.string().min(getBingoBoardPathRoomIdMin),
  "weekStart": zod.date()
})

export const GetBingoBoardResponse = zod.object({
  "weekStart": zod.coerce.date(),
  "weekLabel": zod.string(),
  "status": zod.enum(['active', 'past', 'locked']),
  "unlockDate": zod.coerce.date().nullish(),
  "squares": zod.array(zod.object({
  "id": zod.string(),
  "text": zod.string(),
  "intensity": zod.enum(['light', 'medium', 'stretch']),
  "family": zod.string(),
  "completed": zod.boolean(),
  "completedAt": zod.string().nullable(),
  "contributorIds": zod.array(zod.string())
})),
  "lineCount": zod.number(),
  "lineBonusAwarded": zod.boolean(),
  "threeLineBonusAwarded": zod.boolean(),
  "fullBoardBonusAwarded": zod.boolean()
})


/**
 * @summary Suggest a square for a future Bingo board
 */
export const suggestBingoSquarePathRoomIdMin = 4;



export const SuggestBingoSquareParams = zod.object({
  "roomId": zod.coerce.string().min(suggestBingoSquarePathRoomIdMin)
})

export const suggestBingoSquareBodyTextMin = 5;
export const suggestBingoSquareBodyTextMax = 180;



export const SuggestBingoSquareBody = zod.object({
  "playerId": zod.string(),
  "text": zod.string().min(suggestBingoSquareBodyTextMin).max(suggestBingoSquareBodyTextMax),
  "intensity": zod.enum(['light', 'medium', 'stretch', 'undecided']).optional()
})

export const SuggestBingoSquareResponse = zod.object({
  "id": zod.string(),
  "playerId": zod.string(),
  "text": zod.string(),
  "intensity": zod.enum(['light', 'medium', 'stretch', 'undecided']),
  "weekStart": zod.coerce.date(),
  "createdAt": zod.string()
})


/**
 * @summary Add a personal quest to a room
 */
export const createQuestPathRoomIdMin = 4;



export const CreateQuestParams = zod.object({
  "roomId": zod.coerce.string().min(createQuestPathRoomIdMin)
})

export const createQuestBodyTitleMax = 180;

export const createQuestBodyNotesMax = 500;




export const CreateQuestBody = zod.object({
  "ownerId": zod.string(),
  "title": zod.string().min(1).max(createQuestBodyTitleMax),
  "notes": zod.string().max(createQuestBodyNotesMax).optional(),
  "dueDate": zod.coerce.date().nullable(),
  "labelId": zod.string().nullish(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})).optional(),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})).optional()
})




export const CreateQuestResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Edit a personal quest
 */
export const updateQuestPathRoomIdMin = 4;



export const UpdateQuestParams = zod.object({
  "roomId": zod.coerce.string().min(updateQuestPathRoomIdMin),
  "questId": zod.coerce.string()
})

export const updateQuestBodyTitleMax = 180;

export const updateQuestBodyNotesMax = 500;




export const UpdateQuestBody = zod.object({
  "ownerId": zod.string().optional(),
  "title": zod.string().min(1).max(updateQuestBodyTitleMax).optional(),
  "notes": zod.string().max(updateQuestBodyNotesMax).optional(),
  "dueDate": zod.coerce.date().nullish(),
  "labelId": zod.string().nullish(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})).optional(),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})).optional()
})




export const UpdateQuestResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Delete a personal quest
 */
export const deleteQuestPathRoomIdMin = 4;



export const DeleteQuestParams = zod.object({
  "roomId": zod.coerce.string().min(deleteQuestPathRoomIdMin),
  "questId": zod.coerce.string()
})

export const DeleteQuestBody = zod.object({
  "ownerId": zod.string()
})

export const DeleteQuestResponse = zod.void()


/**
 * @summary Complete or reopen a personal quest
 */
export const setQuestCompletedPathRoomIdMin = 4;



export const SetQuestCompletedParams = zod.object({
  "roomId": zod.coerce.string().min(setQuestCompletedPathRoomIdMin),
  "questId": zod.coerce.string()
})

export const SetQuestCompletedBody = zod.object({
  "ownerId": zod.string(),
  "completed": zod.boolean()
})




export const SetQuestCompletedResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Set the reward for another player's quest
 */
export const setQuestRewardPathRoomIdMin = 4;



export const SetQuestRewardParams = zod.object({
  "roomId": zod.coerce.string().min(setQuestRewardPathRoomIdMin),
  "questId": zod.coerce.string()
})




export const SetQuestRewardBody = zod.object({
  "actorId": zod.string(),
  "coinValue": zod.number().min(1)
})




export const SetQuestRewardResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Steal half of an eligible overdue quest reward
 */
export const stealQuestRewardPathRoomIdMin = 4;



export const StealQuestRewardParams = zod.object({
  "roomId": zod.coerce.string().min(stealQuestRewardPathRoomIdMin),
  "questId": zod.coerce.string()
})

export const StealQuestRewardBody = zod.object({
  "actorId": zod.string()
})




export const StealQuestRewardResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "title": zod.string(),
  "notes": zod.string().nullable(),
  "dueDate": zod.string().nullable(),
  "labelId": zod.string().nullable(),
  "status": zod.enum(['planned', 'completed']),
  "coinValue": zod.number().nullable(),
  "rewardStatus": zod.enum(['pending', 'assigned']),
  "rewardAssignedBy": zod.string().nullable(),
  "coinsAwarded": zod.boolean(),
  "links": zod.array(zod.object({
  "id": zod.string(),
  "url": zod.string().min(1),
  "name": zod.string()
})),
  "subtasks": zod.array(zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "completed": zod.boolean()
})),
  "stolenCoins": zod.number(),
  "stolenBy": zod.string().nullable(),
  "xpValue": zod.number(),
  "completedAt": zod.string().nullish(),
  "createdAt": zod.string()
})


/**
 * @summary Send a message to the other player
 */
export const createEncouragementPathRoomIdMin = 4;



export const CreateEncouragementParams = zod.object({
  "roomId": zod.coerce.string().min(createEncouragementPathRoomIdMin)
})

export const createEncouragementBodyMessageMax = 300;



export const CreateEncouragementBody = zod.object({
  "fromId": zod.string(),
  "toId": zod.string(),
  "message": zod.string().min(1).max(createEncouragementBodyMessageMax)
})

export const CreateEncouragementResponse = zod.object({
  "id": zod.string(),
  "fromId": zod.string(),
  "toId": zod.string(),
  "message": zod.string(),
  "createdAt": zod.string()
})


/**
 * @summary Create a shared reward
 */
export const createRewardPathRoomIdMin = 4;



export const CreateRewardParams = zod.object({
  "roomId": zod.coerce.string().min(createRewardPathRoomIdMin)
})

export const createRewardBodyTitleMax = 100;

export const createRewardBodyDescriptionMax = 300;

export const createRewardBodyEmojiMax = 4;




export const CreateRewardBody = zod.object({
  "createdBy": zod.string(),
  "title": zod.string().min(1).max(createRewardBodyTitleMax),
  "description": zod.string().max(createRewardBodyDescriptionMax).optional(),
  "emoji": zod.string().max(createRewardBodyEmojiMax).optional(),
  "cost": zod.number().min(1)
})

export const CreateRewardResponse = zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "emoji": zod.string(),
  "cost": zod.number(),
  "createdBy": zod.string(),
  "active": zod.boolean(),
  "createdAt": zod.string()
})


/**
 * @summary Redeem a reward with earned coins
 */
export const redeemRewardPathRoomIdMin = 4;



export const RedeemRewardParams = zod.object({
  "roomId": zod.coerce.string().min(redeemRewardPathRoomIdMin),
  "rewardId": zod.coerce.string()
})

export const RedeemRewardBody = zod.object({
  "playerId": zod.string()
})

export const RedeemRewardResponse = zod.object({
  "id": zod.string(),
  "rewardId": zod.string(),
  "rewardTitle": zod.string(),
  "playerId": zod.string(),
  "providerId": zod.string(),
  "cost": zod.number(),
  "redeemedAt": zod.string(),
  "completedAt": zod.string().nullable(),
  "completedById": zod.string().nullable()
})


/**
 * @summary Edit a shared reward
 */
export const updateRewardPathRoomIdMin = 4;



export const UpdateRewardParams = zod.object({
  "roomId": zod.coerce.string().min(updateRewardPathRoomIdMin),
  "rewardId": zod.coerce.string()
})

export const updateRewardBodyTitleMax = 100;

export const updateRewardBodyDescriptionMax = 300;




export const UpdateRewardBody = zod.object({
  "actorId": zod.string(),
  "title": zod.string().min(1).max(updateRewardBodyTitleMax).optional(),
  "description": zod.string().max(updateRewardBodyDescriptionMax).nullish(),
  "cost": zod.number().min(1).optional()
})

export const UpdateRewardResponse = zod.object({
  "id": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "emoji": zod.string(),
  "cost": zod.number(),
  "createdBy": zod.string(),
  "active": zod.boolean(),
  "createdAt": zod.string()
})


/**
 * @summary Delete a shared reward
 */
export const deleteRewardPathRoomIdMin = 4;



export const DeleteRewardParams = zod.object({
  "roomId": zod.coerce.string().min(deleteRewardPathRoomIdMin),
  "rewardId": zod.coerce.string()
})

export const DeleteRewardBody = zod.object({
  "actorId": zod.string()
})

export const DeleteRewardResponse = zod.void()


/**
 * @summary Create a personal quest label
 */
export const createLabelPathRoomIdMin = 4;



export const CreateLabelParams = zod.object({
  "roomId": zod.coerce.string().min(createLabelPathRoomIdMin)
})

export const createLabelBodyNameMax = 40;

export const createLabelBodyColorMin = 4;
export const createLabelBodyColorMax = 24;



export const CreateLabelBody = zod.object({
  "ownerId": zod.string(),
  "name": zod.string().min(1).max(createLabelBodyNameMax),
  "color": zod.string().min(createLabelBodyColorMin).max(createLabelBodyColorMax)
})

export const CreateLabelResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "name": zod.string(),
  "color": zod.string(),
  "createdAt": zod.string()
})


/**
 * @summary Rename or recolour a personal quest label
 */
export const updateLabelPathRoomIdMin = 4;



export const UpdateLabelParams = zod.object({
  "roomId": zod.coerce.string().min(updateLabelPathRoomIdMin),
  "labelId": zod.coerce.string()
})

export const updateLabelBodyNameMax = 40;

export const updateLabelBodyColorMin = 4;
export const updateLabelBodyColorMax = 24;



export const UpdateLabelBody = zod.object({
  "ownerId": zod.string(),
  "name": zod.string().min(1).max(updateLabelBodyNameMax).optional(),
  "color": zod.string().min(updateLabelBodyColorMin).max(updateLabelBodyColorMax).optional()
})

export const UpdateLabelResponse = zod.object({
  "id": zod.string(),
  "ownerId": zod.string(),
  "name": zod.string(),
  "color": zod.string(),
  "createdAt": zod.string()
})


/**
 * @summary Delete a personal quest label
 */
export const deleteLabelPathRoomIdMin = 4;



export const DeleteLabelParams = zod.object({
  "roomId": zod.coerce.string().min(deleteLabelPathRoomIdMin),
  "labelId": zod.coerce.string()
})

export const DeleteLabelBody = zod.object({
  "ownerId": zod.string()
})

export const DeleteLabelResponse = zod.void()


/**
 * @summary Request a negotiated reward
 */
export const createRewardRequestPathRoomIdMin = 4;



export const CreateRewardRequestParams = zod.object({
  "roomId": zod.coerce.string().min(createRewardRequestPathRoomIdMin)
})

export const createRewardRequestBodyTitleMax = 100;

export const createRewardRequestBodyDescriptionMax = 300;




export const CreateRewardRequestBody = zod.object({
  "requesterId": zod.string(),
  "title": zod.string().min(1).max(createRewardRequestBodyTitleMax),
  "description": zod.string().max(createRewardRequestBodyDescriptionMax).nullish(),
  "offer": zod.number().min(1)
})

export const CreateRewardRequestResponse = zod.object({
  "id": zod.string(),
  "requesterId": zod.string(),
  "responderId": zod.string(),
  "payerId": zod.string(),
  "turnPlayerId": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "offer": zod.number(),
  "status": zod.enum(['awaiting_response', 'countered', 'accepted', 'declined', 'paid']),
  "createdAt": zod.string(),
  "updatedAt": zod.string()
})


/**
 * @summary Respond to a negotiated reward request
 */
export const respondToRewardRequestPathRoomIdMin = 4;



export const RespondToRewardRequestParams = zod.object({
  "roomId": zod.coerce.string().min(respondToRewardRequestPathRoomIdMin),
  "requestId": zod.coerce.string()
})




export const RespondToRewardRequestBody = zod.object({
  "actorId": zod.string(),
  "action": zod.enum(['accept', 'counter', 'decline', 'pay']),
  "offer": zod.number().min(1).optional()
})

export const RespondToRewardRequestResponse = zod.object({
  "id": zod.string(),
  "requesterId": zod.string(),
  "responderId": zod.string(),
  "payerId": zod.string(),
  "turnPlayerId": zod.string(),
  "title": zod.string(),
  "description": zod.string().nullable(),
  "offer": zod.number(),
  "status": zod.enum(['awaiting_response', 'countered', 'accepted', 'declined', 'paid']),
  "createdAt": zod.string(),
  "updatedAt": zod.string()
})
